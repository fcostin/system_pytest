!! a note of caution, written 2 years later
------------------------------------------

This style of testing, while perhaps initially attractive,
becomes something of an anti-pattern if there are multiple
valid ways to represent the expected "correct" output of
a program as text. This can cause many false positives
during regression tests.


For example: suppose the expected output of a program running
a test scenario is the set `E = {1, 2, 3}`. Suppose the program
outputs the set `O = {3, 1, 2}`. Obviously, if we compare the
set `O` with the set `E`, using the natural notion of set
comparison, we will find the two sets to be equal, as the order
of the symbols in the notation is irrelevant. However, if we
compare the specific string representation `"{3, 1, 2}"` of `O`
to the specific string representation `"{1, 2, 3}"` of `E`,
using string comparison, we will declare the two (serialised
representations) of the sets to differ. This is a false positive,
as the difference is only in the irrelevant details of the output
caused by choosing an inappropriate representation.


Ways that this problem may manifest:

*   you find yourself wanting to rewrite the text diff part so that it "ignores timestamps"
*   you find yourself wanting to rewrite the text diff part so that it "ignores line order"
*   you find yourself wanting to rewrite the text diff part so that it "ignores platform-specific path separators"
*   you find yourself wanting to rewrite the text diff part so that it "ignores machine-specific paths"
*   you find yourself wanting to rewrite your program such that it produces output in a sorted order, even though that is not a requirement, just to reduce the number of false positives generated by the test harness


A possible alternative approach: think about a more natural representation of
the output of the program than plain text. Think about how that representation
should be compared, to eliminate false positives. Use this representation
and comparison in code, as standard unit tests, using a unit testing library.
If necessary, restructure your program so that this is possible.
